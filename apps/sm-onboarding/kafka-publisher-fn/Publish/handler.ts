...
import { generateContract } from "../utils/faker";
...
export const PublishHandler = (kafkaClient: KP.KafkaProducerCompact<Contract>): PublishHandler => async (_ctx) =>
    pipe(
      [generateContract()],
      ...
      TE.getOrElseW(errMessage => {
        throw new Error(
          `Error publishing contract|${errMessage}`
        );
      }),
...
// eslint-disable-next-line prefer-arrow/prefer-arrow-functions
export function Publish(kafkaClient: KP.KafkaProducerCompact<Contract>): express.RequestHandler {
